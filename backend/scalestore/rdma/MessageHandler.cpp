...
void MessageHandler::init() {
   ...
   std::cout<<"finished mh init" << std::endl;
   // -------------------------------------------------------------------------------------
};
// -------------------------------------------------------------------------------------
...
void MessageHandler::startThread() {
   for (uint64_t t_i = 0; t_i < FLAGS_messageHandlerThreads; t_i++) {
      std::thread t([&, t_i]() {
         ...
         std::vector<ConnectionContext> mhEndpoints(FLAGS_nodes-1);
         ...
         if (t_i == 0) {
             ...
             for (uint64_t n_i = 0; n_i < FLAGS_nodes; n_i++) {
                 ...
                 mhEndpoints[n_i].response = (rdma::Message*)cm.getGlobalBuffer().allocate(rdma::LARGEST_MESSAGE, CACHE_LINE);
                 mhEndpoints[n_i].request = (rdma::Message*)cm.getGlobalBuffer().allocate(rdma::LARGEST_MESSAGE, CACHE_LINE);
            ...
            finishedInit = true;
             ...
             for(auto mhEndpoint : mhEndpoints )   {
                 ...
                 // fill init messages
                 initMsg->mbOffset = 0;
                 initMsg->plOffset = (uintptr_t)mhEndpoint.response;
                 initMsg->bmId = nodeId;
            ...
            for (uint64_t m_i = 0; m_i < mbPartition.numberMailboxes; m_i++, mailboxIdx++) {
               ...
               switch (ctx.request->type) {
                  ...
                  case MESSAGE_TYPE::CUSFR: {
                      ...
                      bucketManager.enterNewShuffledPidToBucketManager(incomingBucketMessage.shuffledPid);
                      PID shuffledPid = PID(incomingBucketMessage.shuffledPid);
                      auto guard = bm.findFrameOrInsert<CONTENTION_METHOD::BLOCKING>(shuffledPid, Invalidation(), ctx.bmId, true); // todo yuval - think of functor here
