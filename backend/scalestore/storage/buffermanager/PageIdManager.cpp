...
void PageIdManager::initPageIdIvs(){
    ...
    uint64_t partitionSize = maxValue / numPartitions;
    uint64_t rollingIvStart = 0;
    for(uint64_t i = 0; i < numPartitions; i++){
...
void PageIdManager::removePage(uint64_t pageId){
    pageIdSsdMapMtx.lock();
    uint64_t slotToFree = pageIdToSsdSlotMap[pageId];
...
void PageIdManager::prepareForShuffle(){
...
uint64_t PageIdManager::getNewPageId(bool oldRing){
    ...
    while(true){
        ...
        if(lockCheck){
            uint64_t temp;
            temp = tripleHash(chosenPartition->second.iv);
            while(getNodeIdOfPage(temp  ,true) != nodeId){
                temp++;
                temp = tripleHash(temp);
            }
